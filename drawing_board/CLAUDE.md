...
This repo is **Etch** — an audio-first, blind-accessible *nonogram / picross*
logic puzzle built on the `syngen-template` (which sits on the
...
parses. The pure logic — including the line solver, the generator, and the whole
game FSM — can be smoke-tested headlessly by loading
`src/js/content/{events,constants,board,game}.js` into Node with a `content`
global (game.js touches only board/events, no audio/DOM). See `/tmp/etch-smoke.js`,
which unit-tests the line solver, asserts every generated puzzle is uniquely
line-solvable, and drives the FSM to a clear and to game over.

This project already has a game (Etch); it is **not** a blank template, so the
"starting a new game" foundational-questions flow below does **not** apply to
ongoing work here.

## This game: Etch

**Pitch.** A nonogram. Every row and column has a run-length clue; deduce which
cells are filled and fill them to reveal the hidden picture. Every puzzle is
generated to be solvable by **pure logic — no guessing** — so a wrong fill is a
real mistake that costs one of three lives. Clear the picture to climb to a
bigger grid. A *thinking* puzzle, deduction end to end.

**Design decisions (the template's required questions, answered):**

1. **Perspective / movement** — static square grid, turn-based. A cursor moves
   one cell at a time (`content/game.js` owns the cursor + FSM; `content/board.js`
   owns the solution, clues, marks, and the generator). No AI.
2. **Audio listener — STEREO + PITCH, never rotates.** A flat grid the player
   faces; COLUMN → stereo pan (`StereoPannerNode`; col 0 hard left, last col hard
   right), ROW → pitch (top/north high, bottom/south low), MARK → timbre (filled =
   warm tone, crossed = tick, unknown = pip). The template's "stereo /
   non-spatial, turn-based" path — no `engine.position`, no listener yaw, no
   y-flip. Left is always left because nothing rotates.
3. **Audio role — audio-first / blind-accessible.** Playable by ear: the cursor's
   position tone, a per-line **read** (spoken clue + a pan/pitch scan of the
   line's marks), a **clue rhythm** (each run = that many beats), and two
   `aria-live` regions. The visual grid is `aria-hidden` (sighted/low-vision).
4. **Input — keyboard + gamepad.** Move, **Enter** fill, **X** mark-empty,
   **Space** read row, **C** read column. Mouse pointer-lock stays neutered.
5. **Persistence — high scores only** (local `app.highscores` + online
   leaderboard). No `engine.state`/autosave.
6. **Progression — endless score chase with discrete levels.** Grids grow
   (5×5→10×10) (`constants.js: levelConfig`). FSM screens: menu → game ↔ pause,
   game → gameover → menu; plus help, highscores, language, hidden `#learn`/`#test`.
7. **Synth aesthetic — modern / realistic.** Warm triangle+sub filled tone, soft
   pip unknown, tick crossed, ascending clue-rhythm beats, bright line-complete
   chime. No retro waves.

**Content modules (`src/js/content/`):**

- `constants.js` — `levelConfig(level)` (board size + fill density), scoring
  (`SCORE_PER_CELL`, `clearBonus`), lives, `MAX_SCORE`.
- `board.js` — the nonogram engine: clue computation, a **line solver**
  (`solveLine` enumerates a clue's placements consistent with partial knowledge
  and intersects them; `lineSolve` propagates rows+cols to a fixed point), and a
  generator (`init`) that reveals minimal locked "given" cells until the line
  solver finishes — guaranteeing a unique, guess-free puzzle. Plus marks,
  `attemptFill` (auto-crosses + locks a wrong guess), `toggleCross`, line/row/col
  progress + completion, and scan helpers. `_solveLine` / `_lineSolveFromGivens`
  / `_solution` are headless-test hooks.
- `game.js` — run FSM (`play` / `levelclear` / `gameover-pending` / `gameover`),
  cursor, `fillCursor` (scores or costs a life), `crossCursor`, line-complete
  detection, scoring, lives, delayed transitions. Emits events.
- `audio.js` — stereo+pitch `positionTone`, `scanRow`/`scanCol` (read a line's
  marks), `clueRhythm`, fill/cross/mistake/line/clear/over cues, ambient, and
  `testDirection` / `sample*` diagnostics. `panForCol`/`freqForRow` are the maps.
- `events.js` — the shared tiny pubsub.

**Controls (in-game):** arrows / WASD / numpad = move · **Enter** = fill (wrong
fill = mistake, −1 life) · **X** = mark empty (free note) · **Space** = read row
(clue + scan) · **C** = read column (clue + scan) · **F1** status · **F2**
progress · **F3** row clue rhythm · **F4** column clue rhythm · **Esc** = pause.
Gamepad: dpad move, A fill, Y mark-empty, X read-row, B read-column, Start pause.

**The line read is the deduction centrepiece, and fair generation is the key
invariant.** Every shipped puzzle is uniquely solvable by `lineSolve` from its
givens — if you ever change the generator, re-run `/tmp/etch-smoke.js` to confirm
puzzles stay guess-free, or wrong-fill life penalties become unfair.

**Online scores — REGISTRATION PENDING.** `app/onlineScores.js` uses id `etch`
with a placeholder `SECRET`. Register the game at
`https://scores.oriolgomez.com/admin` (display_order `desc`, max_score 1000000,
meta `[{"key":"level","type":"int","min":0,"max":999}]`) and paste the returned
secret over `SECRET`. Until then, submissions fall back to the local high-score
