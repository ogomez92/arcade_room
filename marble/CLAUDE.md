...
**Marble** — an audio-first, blind-accessible tilt-maze game built on the syngen-template (which sits on the [syngen](https://github.com/nicross/syngen) Web Audio engine). You tilt a board to roll a marble through a procedurally generated maze to the exit while avoiding pits. It is endless: clearing a level generates a harder one. Scoring is **time-based** — each level cleared awards points by speed (`content/game.js:levelScore`: `SCORE_BASE * clamp(par/time, …)`, where `par` scales with the solution path length so a bigger maze is worth ~the same at par pace); the run total accumulates and only the high score persists. Ships as an HTML5 app (served from `public/`) and an Electron desktop app (`electron/main.js`).

There is no test suite, no linter, and no `npm` scripts. All tasks run through Gulp. The pure-logic core (maze/physics/tilt) can be smoke-tested headlessly by loading `src/js/content/{constants,player,maze,physics,game}.js` into Node with `content`/`app`/`engine` stubs — see the simulation approach used when the game was built.

This project already has a game (Marble); it is **not** a blank template, so the "starting a new game" foundational-questions flow below does not apply to ongoing work here.

### Design decisions (the answers that shaped the build)

1. **Perspective / movement** — top-down 2D continuous; tilt-momentum physics (the marble keeps rolling, you fight inertia). `content/physics.js`.
2. **Audio listener** — **player-locked**: listener yaw tracks the marble's travel heading, so "front" is wherever it is rolling. Yaw *holds* below `YAW_SPEED_MIN` and is smoothed otherwise, so a near-stopped ball doesn't spin the world. `content/audio.js:updateListener`. (Unusual for a tilt board — flag for playtest; switching to screen-locked is a one-function change there.)
3. **Audio's role** — audio-first / blind-accessible. Visual UI is the shared accessible shell only.
4. **Input** — keyboard + gamepad. Arrows/WASD + left stick feed the tilt axes; `content/game.js:readTilt` maps `app.controls.game()` `{x,y}` onto a screen-space tilt vector. Mouse adapter is left wired but ignored (returns `{}` unless pointer-locked; we don't read its `rotate`/`lookY`). The `turnAxis/turnLeft/turnRight` mappings are kept as **empty arrays** because the keyboard/gamepad adapters call `.reduce()` on them every frame.
5. **Persistence** — high scores only. `app.autosave` is disabled in `main.js`; `app/highscores.js` owns its own `marble-highscores-v1` key (localStorage + Electron file bridge).
6. **Progression** — procedural endless. FSM: `menu → game → (pause ⇄ game) → gameover → menu`. Death waits ~1.1s for the falling sting before the game-over screen (`screen/game.js` `onGameOver` + pending flag).
7. **Synth aesthetic** — modern synth (triangle/saw voices, filtered). Global reverb send is killed in `main.js` (we author per-cue tails).

### Module map (`src/js/content/`)

Loaded alphabetically by Gulp, so every cross-module reference is **lazy** (inside functions): `audio.js` evaluates before `constants.js`.

- `constants.js` — `content.constants`: tuning (gravity/damping/radii/ranges) + `clamp`, `angleDelta`.
- `maze.js` — procedural generation; guarantees a **pit-free BFS path** start→goal (pits scatter off-path). Queries: `isWall/isPit/isGoalCell`, `goalPos`, `nearestPit`, `nextStepToGoal` (radar + F2).
- `player.js` — marble state `{x,y,vx,vy,heading}` + getters.
- `physics.js` — `step(dt, tilt)`: accelerate under tilt, damp, soft-cap, **sub-step** the move (anti-tunnel), resolve **circle-vs-grid** wall collisions (2 passes, restitution), detect `'fell'`/`'goal'`.
- `audio.js` — player-locked listener, non-spatial **rolling self-voice** (pitch/gain/brightness ∝ speed), looping props (`goal` beacon, nearest-`pit` warning, `wall`-ahead probe) via `makeProp` (binaural + behind-muffle), directional **radar tick**, one-shots (`clack`/`fell`/`goal`/`levelStart`), `setStaticListener`, `silenceAll`, `_props` for the learn screen.
- `game.js` — endless FSM/state, `startRun`/`nextLevel`/`update`, `readTilt`, time-based scoring (`levelScore` + per-level `levelTime`/`parTime`; `onLevelClear(fn)` fires the per-level speed-bonus announcement), F-key info (`goalInfo`/`pitInfo`/`relBucketKey`).

New app modules added on top of the template: `app/announce.js` (template shipped none), `app/highscores.js`, and screens `pause`, `gameover`, `help`, `learn`, `test` (the `#test` / `#learn` diagnostic routes are wired in `screenManager.js`'s `none→activate`).

---

## Reference: starting a *different* new game from the bare template

(Retained from the template for when this scaffold is copied to start a fresh game. Not applicable to Marble itself.) When `src/js/content/` is empty and there is no game-specific `CLAUDE.md`, ask these foundational design questions before writing any `content/` code — picking wrong and refactoring later is expensive (especially #2). Skip questions already answered; batch the rest into one message.
