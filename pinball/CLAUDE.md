...
- **Spin events.** Each π of *unsigned* travel (`angleTraveled`) emits one `spin` event — matching the real-life reed switch firing once per blade pass through vertical, regardless of rotation direction. The game module plays the click on every spin and scores 100 each, but defers the screen-reader announce: `state.spinChainCount` tallies silently while the spinner is in motion, then `flushSpinnerChain()` says `"Spinner! N times"` once when every spinner has returned to rest (or when the ball drains mid-chain). Without this, a 6-spin pass would queue six interrupting utterances and tank the screen reader.
