...
- `src/js/app/debugLog.js` — fire-and-forget log sink (see "Debug log infrastructure").
...
Browser DevTools logs aren't visible to Claude when the user is playing on the deployed site. The repo includes a small log-streaming setup so future-Claude can read in-game events directly from a file on disk.

### Architecture

```
browser  ── HTTPS POST ──▶  Caddy (debug.oriolgomez.com)
                                │
                                ▼  reverse_proxy
                          127.0.0.1:8766
                                │
                                ▼
                  tools/log-server.js (Node)
                                │
                                ▼
              pinball-debug.log  (gitignored, in repo root)
```

- `src/js/app/debugLog.js` — defines `app.debugLog(event, data)`, fire-and-forget. POSTs a JSON line to `https://debug.oriolgomez.com/log`. Falls silent after a handful of failures, so gameplay is unaffected if the helper isn't running.
- `tools/log-server.js` — tiny Node HTTP server that listens on `0.0.0.0:8766` and appends each POST body to `pinball-debug.log` (truncating the file on startup). Also accepts `POST /clear` for a one-shot wipe.
- `/etc/caddy/Caddyfile` — entry `debug.oriolgomez.com { reverse_proxy 127.0.0.1:8766 }` provides TLS termination so the browser (which is on HTTPS) doesn't get mixed-content-blocked.

### Operating the log server

Start it (idempotent — the script truncates the log on each start):

```sh
node tools/log-server.js &
```

Verify it's reachable end-to-end:

```sh
curl -s -X POST -d '{"event":"smoke"}' https://debug.oriolgomez.com/log -o /dev/null -w "%{http_code}\n"
# expect 204
```
...
```sh
tail -f pinball-debug.log    # stream
wc -l pinball-debug.log      # how much is there
grep '"launch"' pinball-debug.log    # all plunger releases
grep '"event":"event"' pinball-debug.log | grep '"kind":"bumper"'  # all bumper hits
```

Wipe between sessions:

```sh
curl -s -X POST https://debug.oriolgomez.com/clear -o /dev/null
```

### What's logged

Every call to `app.debugLog(event, data)` writes one JSON object per line. The current call sites:

- `launch` (`game.js`) — fired once on plunger release. Carries `power`, `vy`, `x`, `y`.
- `frame` (`physics.js`) — fired every physics tick (~60 Hz) while the ball is live. Carries `x`, `y`, `vx`, `vy`, `sp` (speed), `op` (1 if on plunger), `gf` (consecutive gutter frames).
- `event` (`physics.js`, `pushEvent`) — every collision: `bumper`, `sling`, `wall`, `target`, `rollover`, `flipperHit`, `flipperBlock`, `drain`, `rearm`. Each carries the `kind` and the contact `x`/`y` (and `id`/`speed`/`strength`/`side` where applicable).
- `drain` / `rearm` (`game.js`) — fired alongside the physics events with the ball's full pose at the moment of resolution.

Each line also has a `t` field in seconds (`engine.time()` at the moment of the call) so a session can be reconstructed end-to-end.

### Tracking down a bug from the log

The two most useful commands when a player reports "the ball does X":

```sh
grep -n '"launch"\|"drain"\|"rearm"' pinball-debug.log    # find the timeline anchors
awk '/"launch"/{found=1; n=0} found{print; n++; if(n==50) exit}' pinball-debug.log    # 50 lines after the most recent launch
```

Look for any `event` whose contact `x,y` falls inside a region where it shouldn't (e.g. `wall` events at `x ≈ 3.6, y ≈ 6.4` revealed the right mid-rail crossing the plunger lane).

### When the helper isn't running

The browser keeps trying for ~10 POSTs, then disables itself for the session. So a stale build that points at the helper has no perceptible effect on a player who isn't currently running it. To re-enable mid-session, reload the page.
