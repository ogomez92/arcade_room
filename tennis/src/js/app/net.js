/**
 * Multiplayer transport. Thin wrapper over PeerJS — same shape as
 * bumper/racing in this collection so the self-hosted coturn server
 * keeps working uniformly. See template CLAUDE.md "Multiplayer"
 * section for the design rationale.
 *
 * Topology: star with a single host and one client (1v1 tennis). The
 * host runs the simulation; the client sends inputs and replays
 * snapshots.
 *
 ...
 *   h→c  {type: 'start'}
 *   h→c  {type: 'snap', ...content.match.snapshot()}
 *   h→c  {type: 'end', reason}
 *   any  {type: 'kick', reason}
 *   any  {type: 'ping'|'pong', t}
 *
 * The room code prefix is 'tennis-' so it doesn't collide with other
...
app.net = (() => {
  ...
  function attachClientConnection(conn) {
    ...
    // Tennis is 1v1, so only one client allowed.
    if (clientConns.size >= 1) {
      conn.on('open', () => {
