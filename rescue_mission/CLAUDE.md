...
This repo is **AIRLIFT** — an audio-first, blind-accessible *rescue-chopper game*
(Choplifter lineage) built on the `syngen-template` (which sits on the
...
`/tmp/airlift-sim.js` does exactly this and plays the run with a rescue AI to
balance the difficulty curve.

This project already has a game (AIRLIFT); it is **not** a blank template, so the
...
**Pitch.** An audio rescue-chopper game (Choplifter lineage). Fly along a strip,
hover over stranded survivors to winch them aboard, ferry them home to base — while
ground tanks shell you from below. Clear a wave's survivors to advance; endless,
escalating; three lives. The collection's first rescue / carry / deliver loop.

**The model (the whole game).** 1-D horizontal strip (BASE at x=0, survivors +
tanks out to `FIELD_W`). The chopper moves L/R only while you hold a key (it hovers
the instant you stop). Holding still over a survivor for `HOVER_TIME` winches them
aboard (up to `CAP`); reaching BASE delivers the load (`deliverBonus` stacks for a
full load). `bomb()` drops straight down, destroying tanks within `BOMB_RADIUS` of
your column — but bombs are **limited** (`BOMB_AMMO_START` + per-wave refills), the
key bound. Tanks run an `idle → aim → shell` FSM; a shell topping out within
`HIT_RADIUS` (narrow, so a dense line leaves gaps) of your column costs a life.
`waveConfig(wave)` is the difficulty knob.

**Design decisions (the template's required questions, answered):**

1. **Perspective / movement** — side-view 1-D strip; fly L/R, hover to pick up,
   bomb down. Continuous loop via the game screen's `onFrame` → `game.update(delta)`.
...
6. **Progression — endless waves / score chase.** FSM screens: menu → game ↔
   ...
   / `#test` routes.
7. **Synth aesthetic — modern / realistic** (warm survivor beacons, a low base hum,
   sawtooth tank rumble + rising aim tell + climbing-shell tone, a highpassed bomb +
   sub-bass impact, a pulsing rotor bed). No retro waves.

**Content modules (`src/js/content/`):**

- `constants.js` — `FIELD_W` / `BASE_X` / `MOVE_SPEED`, `CAP` / `PICKUP_RADIUS` /
  `HOVER_TIME`, `BOMB_*` (incl. `BOMB_AMMO_START` / `_PER_WAVE`), `RISE_TIME` /
  `HIT_RADIUS` / `INVULN`, `waveConfig(wave)` (survivors / tanks / fire rate / aim),
  scoring (`deliverBonus` / `TANK_POINTS` / `waveBonus`), `MAX_SCORE`. **The balance
  knob.**
- `game.js` — the real-time run: chopper move + hover-winch + base-deliver,
  `bomb()` (ammo-gated), tank `idle/aim/shell` FSM + hit resolution, falling bombs,
  lives / invuln / wave / game-over. `nearest(kind)` for the F-keys. Emits events.
  No DOM/audio refs → headless under `/tmp/airlift-sim.js`. (Snapshot gotcha: ammo
  is exposed as `ammo`, not `bombs` — `bombs` is the falling-bomb list.)
- `audio.js` — side-view stereo voices (no binaural): `survivor(dx)` / `base` /
  `tankBlip` / `tankAim` / `tankFire` / `shellTop`, `bombDrop` / `bombImpact` /
  `tankKilled`, `hover` / `pickup` / `deliver` / `hurt`, the pulsing rotor bed
  (`startAmbient` / `setSpeed`), and the `sample` / `testDirection` diagnostics.
- `events.js` — the shared tiny pubsub.

**Controls (in-game):** **Left / Right** (A/D) fly · hover still over a survivor to
winch · fly to BASE to deliver · **Space / Down** bomb · **F1** status · **F2**
nearest survivor · **F3** nearest tank · **F4** load/left/base · **Esc** pause.

**Online scores — REGISTRATION PENDING.** `app/onlineScores.js` uses id `airlift`
with a placeholder `SECRET`. Register the game at
`https://scores.oriolgomez.com/admin` (display_order `desc`, max_score 9999999,
meta `[{"key":"wave","type":"int","min":0,"max":999}]`) and paste the returned
