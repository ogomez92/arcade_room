...
- **`audio.js`** — spatial-audio orchestration. Builds looping binaural props for ghosts, fruit, walls, and one per power pellet (each pellet's loop plays at its tile and goes silent the instant `maze.getCell` flips it from `'power'` to `'empty'`). The four power-pellet loops use distinct (root, overtone, tremolo) configs from `POWER_PELLET_CONFIGS`, indexed in row-major scan order so the corners get NW → NE → SW → SE and pitch+tremolo climb in reading order — this is a *spatial-awareness* cue (low-and-slow = top-left, high-and-fast = bottom-right), not just decoration. They're also exposed under `props.powerNW/NE/SW/SE` so the Learn Sounds screen can preview each one. Runs the radar beacon (BFS-pathfinds every 1.5 s and emits a tick at the next path step toward the current target — fruit takes precedence over the nearest dot while it's on the board). Applies a global "behind" muffle (per-source lowpass driven by `behindness()`), and drives the listener pose from Pac-Man each frame. All continuous loops sit at the source's actual tile — the player needs a stable spatial picture; the BFS-routed beacon is the cue for "go this way to actually reach it." **Coordinate convention is non-obvious — see Gotchas below.**
...
- **Ctrl+Alt+D** — toggle "ghosts off" debug mode (freezes ghost AI, skips ghost collision in `game.checkCollisions`, force-mutes per-ghost / frightened / eyes spatial loops in `audio.frame`). Edge-detected on D so a held chord doesn't flap the toggle.
- **Esc** — pause
