app.screenManager = (() => {
  ...
  const machine = engine.tool.fsm.create({
    ...
    transition: {
      none: {
        activate: function () {
          // URL hash routing: #test or #music goes straight to the matching
          // diagnostic screen; anything else lands on the main menu.
          const hash = (window.location.hash || '').replace(/^#/, '')
          const target = (hash === 'test' || hash === 'music') ? hash : 'menu'
          this.change(target)
        },
      },
    },
