...
This repo is **COIL** — an audio-first, blind-accessible *Snake* built on the
`syngen-template` (which sits on the [syngen](https://github.com/nicross/syngen)
Web Audio engine). Ships as an HTML5 app served from `public/` (also
Electron-capable). The template scaffolding docs are retained below the game
section.
...
parses. The pure logic can be smoke-tested headlessly by loading
`src/js/content/{events,constants,game}.js` into Node with a `content` global —
`/tmp/coil-sim.js` does exactly this and plays the run with a steering AI to
balance the difficulty curve.

This project already has a game (COIL); it is **not** a blank template, so the
"starting a new game" foundational-questions flow below does **not** apply to
ongoing work here.

## This game: COIL

Full design doc: **`docs/GDD.md`**. Summary below.

**Pitch.** An audio Snake. You steer a serpent around a walled board, eating food
to grow longer and faster — and your own lengthening body becomes the maze you
must not bite. Endless, escalating speed, three lives, score chase.

**The model (the whole game).** Tile grid, walled border. The snake auto-advances
one cell per step (`stepFor(eaten)` shrinks the step as you eat — faster). You set
an absolute heading (Up = north; no 180° reversal). Eating food grows you +1,
speeds up, scores, respawns food. Crashing into a wall or your own body costs a
life. The signature blind cue is the **clearance cage**: each step the game
reports every blocked non-rear neighbour, and the screen ticks a harsh note from
each one's absolute direction — so a coiling snake hears its cage tighten and
steers to the silent gap. A food beacon pans toward the food.

**Design decisions (the template's required questions, answered):**

1. **Perspective / movement** — top-down 2D tile grid; auto-advance + set heading.
   Continuous loop via the game screen's `onFrame` → `content.game.update(delta)`.
2. **Audio listener — SCREEN-LOCKED, STEREO (not binaural).** Originally binaural
   head-relative, but its HRTF was too subtle here (a source at east-4 collapsed to
   centre), so it was switched to a plain `StereoPannerNode` per source: `pan =
   dx/hypot(dx,dy)` (east hard right, west hard left). The front/back axis stereo
   can't carry is encoded separately — sounds BEHIND you (south, `behindFor>0`) get
   a lowpass muffle + a slight pitch droop. Nothing rotates; +x=east=right,
   +y=south=behind, -y=north=ahead. No `engine.position`/`engine.ear` at all.
...
7. **Synth aesthetic — modern / realistic** (pitched slither tick, harsh
   blocked-cage ticks, a bright food beacon, a juicy eat chomp, a low crash, a
   soft ambient bed). No retro waves.

**Content modules (`src/js/content/`):**

- `constants.js` — `W`/`H`, `DIRS` (n/e/s/w + `opp`), `START_LEN`, `stepFor(eaten)`
  (the pace curve — the difficulty knob), `foodPoints(eaten)`, `MAX_SCORE`.
  **This is the balance knob** — see `docs/GDD.md`.
- `game.js` — the real-time run: snake body + occupancy set, `setDir` (buffered,
  no reversal), `advance()` (eat / grow / move / crash), food spawning, lives /
  respawn / game-over phases. Emits a `step` event each move carrying the blocked
  non-rear neighbours (`warns`) + the food vector — the screen renders audio from
  it. No DOM/audio refs → headless under `/tmp/coil-sim.js`.
- `audio.js` — STEREO placement via `panFor(dx,dy)` + `behindFor(dx,dy)` (muffle +
  pitch droop for behind); `spatialAt(dx,dy,build)` wraps a one-shot in a muffle →
  `StereoPannerNode`. The clearance cage (`setCage([{dir,dist}])`) runs one
  timer-driven ping train per blocked side (`makeCageVoice`) — short broadband ticks,
  faster/louder/sharper the closer the blocker, hard-panned to its side. Plus `step` /
  `foodBeacon` / `eat` / `crash` / UI cues, the ambient bed, and the `sample` /
  `testDirection` diagnostics. (`blocked(dir)` is now just a learn-screen demo of one
  point-blank cage ping.)
- `events.js` — the shared tiny pubsub.

**Controls (in-game):** **Arrow keys / WASD** turn (absolute, Up = north) · **F1**
status · **F2** food direction · **F3** blocked sides · **F4** heading · **Esc** pause.

**Online scores — REGISTRATION PENDING.** `app/onlineScores.js` uses id `coil`
with a placeholder `SECRET`. Register the game at
`https://scores.oriolgomez.com/admin` (display_order `desc`, max_score 9999999,
meta `[{"key":"wave","type":"int","min":0,"max":999}]` — `wave` is food eaten) and
paste the returned secret over `SECRET`. Until then, submissions fall back to the
local high-score table. See `/home/scores/INTEGRATION.md`.
