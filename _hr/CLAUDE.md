...
## Horse Race! — game-specific (READ THIS FIRST)

Audio-first horse-racing game. Player rides a horse against AI (single
player) or other humans (multiplayer over PeerJS + the shared coturn
server). 1-kilometre track, six fences along the way, stamina-managed
sprint vs rest dynamic, jump timing windows. Built on top of
`syngen-template` and reuses its patterns. **The foundational design
questions in the next section have already been answered — do NOT
re-ask them.**
...
| Decision | Answer |
|---|---|
| Perspective / movement | First-person on horseback; 1D track (forward x), lane y for stereo separation only |
| Audio listener | **Player-locked binaural** (yaw=0, listener moves with player horse). Other horses spatialized; behind-listener muffle via per-source lowpass. |
| Audio role | Audio-first / blind-accessible — visuals are minimal status text |
| Input | Keyboard primary (Space=whip, ArrowUp=jump, F1–F4 status hotkeys, Esc to leave) |
| Stamina / speed | Whip costs stamina (more when tired); whip effectiveness scales with stamina; rest > REST_THRESHOLD (0.7s) recovers fast, busy whipping recovers slow; ceiling drops in `tired`/`exhausted` brackets |
| Jumps | One jump key. Air phase 0.75s. Perfect lead = 0.42s before fence; ±0.10s = perfect, ±0.32s = clean, outside = early/late → crash on contact |
| Win | First across `TRACK_LENGTH = 1000m`. Race lingers `FINISH_HOLD = 4s` after the player's finish so trailing horses can also cross |
| Solo | 5-horse race (player + 4 AI), single race; no persistence yet — score shown on the gameover screen |
| Multiplayer | PeerJS broker + `turn.oriolgomez.com` (slug `horserace-`). Up to 6 horses. Host-authoritative, 20Hz snapshots, host fills empty slots with AI |
| Audio | Pure procedural synth — hooves (brown noise + per-stride env), breath (alternating BP white noise), whip (HP white noise burst), fence beacon (pulsing 78 Hz square), crash (low thump + dissonant chord + whinny), finish bell (harmonic stack), start gun (HP burst + low body) |
| Commentator | Bilingual EN/ES via `app.i18n` dictionaries. Two aria-live regions (`.js-announcer` polite, `.js-announcer-assertive` assertive) plus `.js-lobby-announcer` |

### File layout

```
src/js/content/
  race.js         — shared state: horses, phase, ranking, results, listeners
  obstacles.js    — seeded fence layout + jump timing evaluator
  player.js       — local input buffer; whip/jump appliers; physicsStep (shared with AI)
  ai.js           — per-AI personalities with hysteresis on whip-or-rest
  audio.js        — listener pose, per-horse hooves+breath voices, beacon voices, crowd, one-shots
  announcer.js    — stamina-bucket transitions, overtake/being-overtaken cues, F-key readouts, race events
  game.js         — top-level FSM: setupSinglePlayer / setupMultiplayerHost / setupMultiplayerClient, tick, snapshot/applySnapshot

src/js/app/
  announce.js     — aria-live funnel
  i18n.js         — EN+ES, STORAGE_KEY = 'horserace.lang'
  net.js          — PeerJS wrapper, turn config, MAX_CLIENTS=5
  screen/{splash,menu,language,help,learn,lobby,game,gameover}.js
```

### Wire protocol (multiplayer)

JSON over data channel:
- `c→h  {type:'hello', name}`
- `c→h  {type:'input', whip:bool, jump:bool}` — per keypress
- `c→h  {type:'leave'}`
- `h→c  {type:'lobby', peers:[{peerId, name, isHost}]}`
- `h→c  {type:'start', seed, lineup, mySlot}` — sent once, mySlot tells the client which horse it owns
- `h→c  {type:'snap', t, snap:{phase, raceTime, countdownLeft, horses:[{slot,x,y,speed,stamina,airborne,airTime,crashed,finishedAt,rank,cleanJumps,perfectJumps,crashes}]}}` — 20Hz
- `any  {type:'kick'|'ping'|'pong'}`

Snapshots replicate authoritative state. Clients dead-reckon their
own player from local input for low latency, then soft-correct from
the snap if drift > 4m. Other horses (AI + remote humans) move only
via snapshots on the client.

### Coordinate frame

Race-world coords directly (no screen→audio flip — this game has no
screen-y to flip). Player horse sits at `(myX, myY=0)`, other horses
at `(theirX, theirY=±2,±4)`. Listener faces +x (yaw=0). syngen's
binaural means +x=forward, +y=LEFT ear; lane lateral offsets give
stereo separation. Audio's `spatialParams(relX, relY)` returns pan
(`-relY/dist`), distance gain (`1/(1+dist/22)`) and behind-cutoff
(18 kHz → 700 Hz as the source rotates behind).

### Scoring

```
positionPts:  [1000, 600, 400, 250, 150, 100, 60, 40][rank-1] || 20
jumpPts:      cleanJumps*120 + perfectJumps*250
crashPenalty: crashes*100
speedBonus:   round(avgSpeed * 30)
staminaBonus: round(avgStamina * 200)   ← rewards smart pacing
timeBonus:    max(0, round((100 - finishTime) * 8))
total = positionPts + jumpPts - crashPenalty + speedBonus + staminaBonus + timeBonus
```

### Things to remember

- **Don't broadcast events out-of-band.** Audio events for other
  horses (whip cracks, jump whooshes, crash thuds) are triggered by
  `applyWhip/applyJump/physicsStep` on the **host** — clients hear
  them by replaying the same effect at their snap-corrected positions.
  Don't add a separate audio-event relay queue unless you're hitting
  perceptible mismatches.
- **AI uses player physics.** `content.ai.tick` calls
  `content.player.physicsStep` directly so a balance change to drag
  or whip boost applies to AI too. Don't fork the physics.
- **Listener is sticky.** `engine.position.setVector` persists between
  frames. The `learn` and (future) `test` screens must call
  `content.audio.setStaticListener(true)` on enter, since the game
  screen's `content.audio.frame()` keeps writing the listener pose
  from the player horse.
- **Snap drops in transition gaps are fine.** Between lobby
  detachNetListeners and game attachNet there's a brief window where
  no `message` listener is attached. A snap arriving in that window
  is silently dropped — the next snap (within 50ms) recovers.
- **PEER_ID_PREFIX is `horserace-`.** Public PeerJS broker — codes
  are global. Don't shorten.
