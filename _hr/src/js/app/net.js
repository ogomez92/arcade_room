/**
 * Multiplayer transport. PeerJS over the shared coturn server (see
 * template CLAUDE.md "Multiplayer" section). Star topology with one
 * host and up to N clients (the host fills missing slots with AI).
 *
 * Wire protocol (JSON over the data channel):
 *   c→h  {type: 'hello', name}
 *   c→h  {type: 'input', whip, jump}
 *   c→h  {type: 'leave'}
 *   h→c  {type: 'lobby', peers: [{peerId, name, isHost}]}
 *   h→c  {type: 'start', seed, lineup, mySlot}
 *   h→c  {type: 'snap', t, snap: {...}}
 *   h→c  {type: 'end'}
 *   any  {type: 'kick', reason}
 *   any  {type: 'ping'|'pong', t}
 *
 * Room codes use the unambiguous CODE_CHARSET (no 0/O/I/L/vowels) so
 * players can read them aloud over voice chat.
 */
app.net = (() => {
  ...
  async function host({name = 'Host', code} = {}) {
    ...
    return new Promise((resolve, reject) => {
      ...
      try { peer = new window.Peer(id, peerOptions()) }
      ...
      peer.on('connection', (conn) => attachClientConnection(conn))
  ...
  function attachClientConnection(conn) {
    ...
    if (clientConns.size >= MAX_CLIENTS) {
      conn.on('open', () => {
    ...
    conn.on('open', () => { record.lastSeen = Date.now() })
    conn.on('data', (msg) => {
  ...
  async function join({name = 'Player', code}) {
    ...
    return new Promise((resolve, reject) => {
      let settled = false
      try { peer = new window.Peer(peerOptions()) }
      catch (e) { reject(e); return }
  ...
  function broadcast(msg) {
    ...
    if (role === 'client') return safeSend(hostConn, msg) ? 1 : 0
    return 0
  }
