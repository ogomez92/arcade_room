/**
 * HAMMER OF GLORY! — announcer.
 *
 * Two aria-live regions, polite and assertive. Identical-string
 * swallowing is worked around per the bumper/pacman pattern: clear
 * the region, yield via requestAnimationFrame, then re-set.
 *
 * Optional TTS fallback off by default.
 */
app.announce = (() => {
  let politeEl = null
  ...
  function setText(el, str, prevSame) {
    ...
    if (prevSame) {
      el.textContent = ''
      window.requestAnimationFrame(() => { el.textContent = str })
    } else {
      el.textContent = str
    }
  }
