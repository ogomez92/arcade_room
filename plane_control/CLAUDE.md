...
This repo is **Approach** — an audio-first, blind-accessible air-traffic-control game built on the `syngen-template` (which sits on the [syngen](https://github.com/nicross/syngen) Web Audio engine). Ships as an HTML5 app served from `public/` (also Electron-capable). The template scaffolding docs are retained below the game section.

There is no test suite, no linter, and no `npm` scripts. All tasks run through Gulp (use **pnpm/pnpx**). Build with `pnpx gulp build` (`--debug` for unminified). Do not start a dev server here; verification stops at build/static checks — `node --check public/scripts.min.js` confirms the bundle parses, and `/tmp/approach_harness.js` (re-create from the recipe below) drives the logic headless.
...
- **Premise.** You are the control tower. Planes arrive from the edges of a radar field as spatial engine voices; vector each one to the single runway at the centre before its fuel runs out, and never let two get within separation distance — a mid-air collision or a fuel-out ends the shift. Endless, escalating; score = planes landed (+ a fuel-efficiency bonus).
- **Perspective / movement.** Top-down 2D radar, fixed camera, continuous plane motion. There is **no player avatar** — you command, you don't move. The airspace is a 41×41 field (`constants.GRID`); fractional `(col,row)`, `row` south+, `col` east+. The tower + runway sit dead-centre at `TOWER` (20,20).
- **Audio listener: SCREEN-LOCKED, pinned at the tower.** The listener never moves or rotates (`audio.setStaticListener`, yaw 0); `audio.playerPos()` returns `TOWER`, not a moving player. A plane at world `(col,row)` maps to the binaural frame (`+x`=forward, `+y`=LEFT) as `{x: TOWER.row-row, y: TOWER.col-col}` (`offsetAudio`/`relAudio`). North→front, east→right, south→behind (muffled + detuned via `behindness`+`applyBehind`), west→left. Every spatial voice ALSO rides a `StereoPannerNode` (`panFor`, bearing-sine pan) summed with the binaural ear — the dominant L/R cue. Coords passed to audio are raw tile units (no meter scaling). **Verify with `#test` after any listener/coord change.**
- **Audio-first.** Fully playable by ear; the visible UI is the accessible menu shell only. Per-plane pitch jitter (`audio.jitter(116, id)`) keeps planes distinguishable; the selected plane is brightened + louder (`planeVoice` `setSelected`); low fuel adds an escalating warble (`setUrgency`).
- **Synth aesthetic: modern.** Each cue authors its own ADSR + lowpass tail; the global syngen reverb send is disabled in `main.js`.
- **Input: keyboard-first.** Raw `keydown` in `app/screen/game.js`. **Cycle + command** model: `Tab`/`Shift+Tab` select next/prev plane; `Left`/`Right` turn the selected plane by `TURN_STEP` (30°); `Up` vectors it direct to the field; `L`/`Enter` clears it to land; `H` holds (orbit); `Space` reads the selected plane (bearing/distance/fuel); `R` reads shift status; `P` pause; `Esc` menu; `F1`/`F2` redundant status reads.
- **Persistence: high scores only.** Local per-difficulty top-10 (`app/highscores.js`, key `approach-highscores-v1`) + soft online submit to `scores.oriolgomez.com` (`app/onlineScores.js`, GAME_ID `approach`). Online fails soft until the game id + matching SECRET are registered with the admin endpoint (placeholder secret in the bundle). Difficulty in `localStorage 'approach.difficulty'`, nickname in `'approach.nickname'`, locale in `'approach.lang'`.
- **Difficulties: cadet / controller / nightmare**, all unlocked (`DIFFICULTY_TABLE` + `levelParams(difficulty, elapsedSeconds)` — faster planes, less fuel, busier skies, shorter spawn intervals as the shift goes on).

### Plane lifecycle + the core rules (`content/planes.js`, states in `constants.PLANE`)
States: `ENROUTE` (flies on its entry heading / last turn) · `HOLDING` (orbits in place, still burns fuel — the primary **spacing** tool) · `CLEARED` (the tower **auto-vectors it to the runway and lands it** — see below) · `LANDED` (scored + removed). (`PLANE.FINAL` exists in the enum but is unused after the manual-approach experiment was reverted — landing no longer needs a separate state.)

**Why auto-vector, not hand-flying.** The listener is pinned at the tower, so a plane's voice tells you its *position* (bearing + distance) but NOT its *heading* — heading is inaudible. Hand-flying a plane onto the runway by ear is therefore unfair, so `clearToLand` is the whole landing act: it auto-vectors the plane straight to the tower and it touches down on arrival. The skill is **ear-playable**: decide WHO to clear and WHEN (single runway), and HOLD + space everyone else. `turn`/`directToTower` cancel clearance (go-around) and are a secondary nudge for breaking up a developing conflict, not required for the core loop.

**Single runway.** Only one plane may be `CLEARED` at a time; a second `clearToLand` is refused (`commandReject` + `runwayBusy`). The cleared plane is **exempt from separation only once inside `RUNWAY.finalRadius`** (the protected approach corridor — what stops a false collision as it crosses the busy centre). **Separation** (`SEP.crash` → collision game over; `SEP.warn` → throttled `conflictAlert` klaxon + chatter) is checked pairwise among all airborne planes except that on-final occupant. **Fuel** burns every airborne frame; 0 = fuel-out game over. Planes never leave radar — `airspace.keepIn` banks them back — so the only losses are collision and fuel-out (the locked failure modes). Skies start busy (`startPlanes` per difficulty) and saturate as `levelParams(difficulty, elapsedSeconds)` shortens spawn intervals and raises the concurrent cap; arrivals retry their entry point up to 12× to avoid spawning inside another plane's bubble. Plane voices are **noise-based** (rumble + airflow + faint sub, no oscillators → no tonal "bee" buzz); per-plane identity is filter colour, not pitch.

### Module map (`src/js/content/`)
`constants` (data/enums/tables, `TOWER`, `levelParams`) · `state` (single career tier: score, landed, plane list, `selectedId`, `runwayOccupant`, elapsed/spawn clocks) · `airspace` (geometry: bounds, heading-vector rotate/`headingToward`, `compass`/`bearingFromTower`, `keepIn` boundary, `randomEntry`) · `audio` (screen-locked listener pinned at tower + all SFX + the looping `planeVoice`) · `planes` (arrivals, steering, fuel, voices, landing, `checkCrash`) · `scoring` (landing award + fuel bonus) · `announcer` (polite/assertive + status hotkeys + optional TTS) · `music` (optional ambient bed, intensifies with traffic) · `game` (top-level FSM + per-frame order + command entry points).

### Game FSM (`content/game.js`, `state.data.fsm`)
`intro → play → gameOver`, plus a manual `paused`. One continuous escalating session — no levels, no level-clear freeze. `play()` advances `elapsed`, runs `music.frame` + `planes.frame`, schedules arrivals against the per-difficulty `maxPlanes` cap, then `planes.checkCrash()` → on a hit, `gameOver(cause, world)` plays the crash sting, silences voices, sets `pendingGameOver`, keeps the tail ticking `GAMEOVER_TAIL_S`, then dispatches the `gameover` screen.

### Verification harness
Without a browser, stub `engine`/`app`/Web-Audio, eval the `content/` modules in **gulp concat order** (alphabetical: airspace, announcer, audio, constants, game, music, planes, scoring, state — do NOT eval `content.js`, its `const content={}` shadows the sandbox global), then drive `content.game` through `startCareer` → frames → `directToTower`/`clearToLand` (lands) → fuel-drain (game over). The screen-layer `app.screenManager.dispatch('gameover')` will throw in the stub (no screens) but `game.frame`'s try/catch absorbs it — that's expected, not a bug. Recreate under `/tmp`.
