// Game over: shows the run summary, the local per-difficulty leaderboard, and
// (when the score qualifies) a name form that saves locally and submits to the
// online board. "Play again" restarts the same difficulty.
app.screen.gameover = app.screenManager.invent({
  ...
  onEnter: function () {
    ...
    if (s.summaryEl) s.summaryEl.textContent = app.i18n.t('gameover.summary', {landed: sum.landed, score: sum.score})
  ...
  saveScore: function () {
    ...
    try { localStorage.setItem('approach.nickname', name) } catch (e) {}
    app.highscores.add(name, s.summary.score, {difficulty: s.summary.difficulty, level: s.summary.landed})
