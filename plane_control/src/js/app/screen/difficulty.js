...
app.screen.difficulty = app.screenManager.invent({
  ...
  transitions: {
    cadet: function () { setDifficultyChoice('cadet'); this.change('game') },
    controller: function () { setDifficultyChoice('controller'); this.change('game') },
    nightmare: function () { setDifficultyChoice('nightmare'); this.change('game') },
...
function setDifficultyChoice(d) {
  try { localStorage.setItem('approach.difficulty', d) } catch (e) {}
}
function getDifficultyChoice() {
  try { return localStorage.getItem('approach.difficulty') || 'controller' } catch (e) { return 'controller' }
}
