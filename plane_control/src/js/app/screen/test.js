// Orientation diagnostic. Listener fixed at the tower facing north. Cycles a
...
// Ticks are placed in WORLD coords around the tower, going through the same
// relAudio() mapping gameplay uses.
app.screen.test = app.screenManager.invent({
  ...
  onEnter: function () {
    ...
    const t = content.constants.TOWER
    const positions = [
      {col: t.col, row: t.row - 5, key: 'test.front'},
      {col: t.col + 5, row: t.row, key: 'test.right'},
      {col: t.col, row: t.row + 5, key: 'test.behind'},
      {col: t.col - 5, row: t.row, key: 'test.left'},
    ]
