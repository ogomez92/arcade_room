...
// sorted descending. Entries: {name, score, level, date} where `level` stores
// the number of planes landed in the run.
app.highscores = (() => {
  const KEY = 'approach-highscores-v1'
  const MAX = 10
  const DIFFS = ['cadet', 'controller', 'nightmare']

  let cache = null

  function blank() { return {cadet: [], controller: [], nightmare: []} }
  ...
  function diffOf(d) { return DIFFS.indexOf(d) >= 0 ? d : 'controller' }
