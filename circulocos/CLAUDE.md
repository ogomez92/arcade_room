...
This repo is **Vault** — an audio-first, blind-accessible *peg solitaire* puzzle
...
parses. The pure logic — including the **whole game FSM** — can be smoke-tested
headlessly by loading `src/js/content/{events,constants,board,game}.js` into Node
with a `content` global (game.js touches only board/events, no audio/DOM). See
`/tmp/vault-smoke.js`, which also DFS-solves generated boards to prove they are
solvable and drives the FSM to a level clear.

This project already has a game (Vault); it is **not** a blank template, so the
"starting a new game" foundational-questions flow below does **not** apply to
ongoing work here.

## This game: Vault

**Pitch.** Peg solitaire. A peg vaults over an orthogonally-adjacent peg into the
empty hole two cells beyond — north, east, south, or west — removing the jumped
peg. Thin the board down to a single peg to clear the level and climb to a larger,
fuller one. Every board is generated solvable. Undo is limited; run dry with no
jumps left and the board is failed (−1 of 3 lives). A pure *thinking* puzzle.

**Design decisions (the template's required questions, answered):**

1. **Perspective / movement** — static square board, turn-based. A cursor moves
   one cell at a time; a move is (peg, direction) (`content/game.js` owns the
   cursor + run FSM; `content/board.js` owns the grid, jump rules, undo, and the
   reverse generator). No AI — single-player puzzle.
2. **Audio listener — SCREEN-LOCKED (fixed yaw), the canonical compass model.**
   North is always front, south behind, east right, west left. Listener parked at
   the origin with `LISTENER_YAW = Math.PI/2`; directional cues emit at compass
   *offsets* from the cursor. Peg solitaire is jumps in the four compass
   directions, so the scan and the jump's hop come from their true bearing — the
   directional audio carries the move information. Screen→audio y-flip is in
   `relativeVector`/`behindness` per the template gotcha.
3. **Audio role — audio-first / blind-accessible.** Playable by ear via the
   directional **scan** (peg/hole/edge per neighbour + a "jumpable" ping), the
   **jump** hop, per-cell announcements, a legal-jump list (F2), and two
   `aria-live` regions. The visual board is `aria-hidden` (sighted/low-vision).
4. **Input — keyboard + gamepad.** Move with arrows; **select a peg (Enter) then
   press a direction** to jump, or **Shift+direction** as a one-step shortcut;
   **U** undoes. Mouse pointer-lock stays neutered.
5. **Persistence — high scores only** (local `app.highscores` + online
   leaderboard). No `engine.state`/autosave.
6. **Progression — endless score chase with discrete levels.** Boards grow
   (5×5→7×7) and gain pegs (`constants.js: levelConfig`). FSM screens: menu →
   game ↔ pause, game → gameover → menu; plus help, highscores, language, and the
   hidden `#learn` / `#test` routes.
7. **Synth aesthetic — modern / realistic.** Warm triangle+sub peg, soft sine
   hole pip, dull-thud edge, a pitched whoosh + capture pop for the jump, bright
   arpeggio clear, somber fail. No retro waves.

**Content modules (`src/js/content/`):**

- `constants.js` — `levelConfig(level)` (board size, peg count, undo budget),
  `clearBonus`, `SCORE_PER_PEG`, lives, `MAX_SCORE`.
- `board.js` — the grid: four-direction `canJump`/`jumpsFrom`/`legalJumps`,
  `jump` + `undo` (move records), `describe`, and the **reverse generator**
  (`generate`: seed one peg, then repeatedly un-jump — the exact inverse of a
  legal jump — so the board is guaranteed solvable back to one peg).
- `game.js` — run FSM (`play` / `levelclear` / `roundfail` / `gameover`), cursor,
  selection, `jumpFrom`, limited `undo`, stuck detection, scoring, lives, delayed
  transitions (flag-guarded). Emits events; the **screen** turns them into audio +
  announcements. No AI module.
- `audio.js` — the screen-locked binaural compass, `positionTone`,
  `scanNeighbors` (the deduction scan), `jumpSound`, select/undo/stuck cues,
  clear/fail/over/level cues, ambient, and `testDirection` / `sample*` diagnostics.
- `events.js` — the shared tiny pubsub.

**Controls (in-game):** arrows / WASD / numpad = move cursor · **Enter** = select
peg (then a direction jumps) · **Shift+direction** = one-step jump · **U** = undo
· **Space** = scan neighbours · **C** = describe cell · **F1** status · **F2**
list legal jumps · **F3** pegs+undos · **F4** last jump · **Esc** = pause.
Gamepad: dpad move/jump, A select, X scan, Y undo, B describe, Start pause.

**The scan + the solvable generator are the heart.** `scanNeighbors` plays, per
compass direction, the neighbour's content and a bright ping if a jump is
available that way — so you plan moves by ear. The reverse generator guarantees
no board is a dead end from the start; getting stuck is a player choice that undo
can walk back (within budget).

**Online scores — REGISTRATION PENDING.** `app/onlineScores.js` uses id `vault`
