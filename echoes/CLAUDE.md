...
This repo is **Echoes** — an audio-first, blind-accessible **sound-memory /
concentration** game built on the `syngen-template` (which sits on the
...
global and driving `setCursor()`/`flipCursor()`/`update()`.

This project already has a game (Echoes); it is **not** a blank template, so the
"starting a new game" foundational-questions flow below does **not** apply to
ongoing work here.

## This game: Echoes

**Pitch.** A grid of cells each hides a sound. Flip one to hear its tone, flip a
second to try to match it. Two cells with the same sound stay revealed; a mismatch
shows briefly then hides again — so remember *where* each sound was. Match every
pair to clear the board and advance to a bigger one. Each flip spends from a
per-level budget; run out before clearing and the run ends. A *thinking* (memory)
game.

**Design decisions (the template's required questions, answered):**

1. **Perspective / movement** — top-down 2D tile grid, turn-based. Cursor moves
   one cell at a time (`content/game.js` owns cursor + flip state; `content/board.js`
   owns the shuffled deck of paired ids).
2. **Audio listener — world-fixed, STEREO (not binaural).** A memory game's job is
   to identify a *timbre*, and pitch-shifting a source for front/back (as a
   binaural median-plane cue would require) would make the two halves of a pair
   sound different — fatal here. So position is carried two ways that never touch
   the timbre: (a) cell sounds are panned by **column** only (west left, east
   right), timbre untouched; (b) a separate **navigation beacon** (`positionTone`)
   encodes the full cell location world-fixed — **pan = column, pitch = row**
   (north high, south low). "North" always sounds the same regardless of anything.
3. **Audio role — audio-first / blind-accessible.** Pairs are matched purely by ear
   (recognising the same instrument+pitch twice). Two `aria-live` regions; the
   visual grid is `aria-hidden` (shows pair letters for sighted players).
4. **Input — keyboard + gamepad.** Arrows/WASD/numpad move; Enter/Space (or gamepad
   A) flip; C describe/replay current cell. Mouse pointer-lock neutered.
...
6. **Progression — endless score chase with discrete levels.** Grid grows
   (`constants.dimsFor`, all even cell counts so no dead cells) up to 4×6 = 12 pairs,
   then the flip budget tightens (`constants.flipBudget`). FSM screens: menu →
   game ↔ pause, game → gameover → menu; plus help, highscores, language, hidden
   `#learn` / `#test`.
7. **Synth aesthetic — modern / clean.** Six instrument families (bell, pluck,
   marimba, glass, reed, bass) × a pitch ladder give each pair a unique signature
   (`audio.timbre(pairId)`). Bright rising chime = match, short falling tone =
   mismatch. Soft pad ambience.

**Content modules (`src/js/content/`):**

- `constants.js` — `dimsFor`/`pairsFor`, `flipBudget` (per-level), scoring
  (`matchScore`, `clearBonus`), `FLIPBACK_DELAY`.
- `board.js` — shuffled deck of paired ids, `describe(x,y)`
  (covered/revealed/matched without leaking unrevealed ids), reveal/hide/match.
- `game.js` — run FSM (`play` / `levelclear` / `gameover-pending` / `gameover`),
  the two-flip attempt state, a `lock` that briefly shows a mismatch before
  hiding, flip-budget accounting + `checkStuck()` game-over, scoring. Emits events;
  the **screen** turns them into audio + announcements.
- `audio.js` — instrument families, `playCell` (column-panned, timbre intact), the
  world-fixed `positionTone` beacon, match/mismatch/clear/over cues, ambience, and
  `testDirection`/`demo*` diagnostics. `setStaticListener` is a no-op (kept so the
  shared `#test` screen doesn't error).
- `events.js` — shared pubsub.

**Controls (in-game):** move = arrows / WASD / numpad / gamepad dpad · **Enter** or
**Space** (gamepad A) = flip · **C** describe/replay current cell · **F1** status
(score/level/flips) · **F2** locate (column/row + beacon) · **F3** pairs left ·
**Esc** pause.

**Why not binaural (important):** see decision #2 — front/back binaural cues would
pitch/colour a source by position, which breaks "same sound = a pair." Keep cell
audio stereo-only; carry row/front-back in the *navigation* beacon, never in the
cell timbre.

**Online scores — REGISTRATION PENDING.** `app/onlineScores.js` uses id `echoes`
