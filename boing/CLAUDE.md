...
## BOING BOING — this game (read me first)

This project is no longer a blank template — it is **Boing Boing**, a shipped
game (its display name; the dir/internal slug is still `aloft`, and the build
artefacts keep that name). The "Starting a new game" design questions below are
already answered (see `docs/GDD.md`); **do not re-ask them**. Boing Boing is an
audio-first, blind-accessible
**vertical bounce platformer** (Doodle-Jump lineage): you bounce upward forever,
gravity pulls you back, and the ONLY steering is left/right — you slide sideways
to line up with the next platform (a panned beacon) so you land on it and bounce
higher. Shoot the floating sentinels. Single life; score = height climbed +
sentinel kills × combo.

Key decisions baked in (don't relitigate without reason):

- **Listener: screen-locked, faces UP the climb, never rotates.** The platform
  you're about to land on is a *beacon*: its **pan** is its horizontal offset
  (steer until it's centred = under you), and it grows louder + ticks faster as
  you fall toward it. `LISTENER_YAW = π/2`; the vertical gap is the "ahead" axis
  fed to `content/audio.js#place()` (compressed so far beacons don't vanish);
  pad-type sets the beacon timbre. Run `#test` after any listener change.
- **The rising void is load-bearing.** A floor rises at `constants.floorRate`,
  snapping to `FALL_MARGIN` below your peak when you climb and creeping up when
  you don't — without it a non-steering player dwells forever (single chain +
  recovery + landing tolerance). It's what forces a climb pace and ends a stuck
  run. Don't remove it; tune `floorRate` / `FALL_MARGIN` instead.
- **Targeting (`content/game.js`): on each bounce, aim for the highest pad below
  the apex** (so springs are used to climb far); on a miss (fallen below the
  target unaligned) re-acquire the best pad at/below you to recover.
- **Audio is the source of truth.** `content/game.js` owns the bounce sim + emits
  events; `app/screen/game.js` reads HELD left/right steering + edge shoot, turns
  events into spatial audio + announcements + an aria-hidden sky viz.
- **Cross-module refs are lazy** (`const K = () => content.constants`) because of
  alphabetical concat (audio.js loads before constants/game). Keep it that way.
- **Online id `boing`, REGISTERED** — real `SECRET` is set in
  `app/onlineScores.js`; scores post to scores.oriolgomez.com (`meta: {level}`)
  and fall back to the local table only on network failure.
- This is a **source-tree game with a Gulp build**: after editing `src/**`, run
  `./node_modules/.bin/gulp build` (or `pnpx gulp build`) to refresh
  `public/scripts.min.js` / `styles.min.css`. The folder is served independently
  — don't start a dev server here.

Everything below is the shared collection/template guidance, kept for reference.
