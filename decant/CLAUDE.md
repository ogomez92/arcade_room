...
This repo is **Decant** — an audio-first, blind-accessible **water-sort puzzle**
built on the `syngen-template` (which sits on the
[syngen](https://github.com/nicross/syngen) Web Audio engine). Ships as an HTML5
app served from `public/` (also Electron-capable). The template scaffolding docs
are retained below the game section.

There is no test suite, no linter, and no `npm` scripts. All tasks run through
Gulp (use **pnpm/pnpx**). Build with `pnpx gulp build` (`--debug` for
unminified). Do not start a dev server in this checkout — verification stops at
build/static checks; `node --check public/scripts.min.js` confirms the bundle
parses. The pure logic (board/game) can be smoke-tested headlessly by loading
`src/js/content/{events,constants,board,game}.js` into Node with a `content`
global and driving `setCursor()`/`select()`/`undo()`/`update()`. The board's
`minSolution()` (BFS) and `solvableNow()` (DFS) make generation and the move
budget verifiable headlessly.

This project already has a game (Decant); it is **not** a blank template, so the
"starting a new game" foundational-questions flow below does **not** apply to
ongoing work here.

## This game: Decant

**Pitch.** A row of vials, each holding a stack of coloured liquid — and every
colour is a different instrument timbre. Pour the top run of one vial onto another
whose top colour matches (or onto an empty one) until every vial is empty or full
of a single colour. You can re-listen to any vial and you can undo, so it is about
*planning*, not memory. Each pour spends a move from a per-level budget; run out
before sorting them all and the run ends. A *thinking* (logic) game.

**Design decisions (the template's required questions, answered):**

1. **Perspective / movement** — static / turn-based. A 1-D cursor moves along the
   row of vials; there is no avatar and no camera. `content/game.js` owns the
   cursor + the source-selection/pour/undo state; `content/board.js` owns the vial
   stacks, the pour rules, the win check, and the solver.
2. **Audio listener — world-fixed, STEREO (not binaural).** There is no listener
   rotation in this game, so pan *is* the spatial truth. A vial's pan is its fixed
   place in the row — leftmost = hard left, rightmost = hard right — and that never
   changes regardless of cursor position. A colour must sound identical wherever it
   is (so you can recognise it), so pan is the *only* thing that varies with
   position; the timbre is never pitch-shifted by location. A pour is the one
   moving cue: a `StereoPanner` ramps from the source vial's pan to the
   destination's so you hear the direction the liquid travelled.
3. **Audio role — audio-first / blind-accessible.** Vials are sorted purely by ear
   (recognising the same instrument and gathering it). Two `aria-live` regions; the
   visual vial row is `aria-hidden` (coloured segment blocks for sighted players).
4. **Input — keyboard + gamepad.** Left/Right (arrows / A,D / numpad / dpad) move
   the cursor; Enter/Space (gamepad A) pick up a source then pour; U/Backspace
   (gamepad B) undo; C (gamepad X) scan the current vial bottom→top. Mouse
   pointer-lock neutered.
5. **Persistence — high scores only** (local + online). No `engine.state`/autosave.
   Local high-score "level" field stores the level reached.
6. **Progression — endless score chase with discrete levels.** Colours grow
   3→4→5→6 (`constants.colorsFor`, capped at 6 for audibility), spare vials drop
   from 2 to 1 at level 6, and the move-budget slack tightens every level. FSM
   screens: menu → game ↔ pause, game → gameover → menu; plus help, highscores,
   language, hidden `#learn` / `#test`.
7. **Synth aesthetic — modern / clean.** Six instrument families (bell, pluck,
   marimba, glass, reed, bass) at fixed, well-separated pitches are the six
   colours. Liquid "whoosh" pours (filtered noise + descending glug), a bright pop
   when a vial is finished, soft pad ambience.

**The move budget is derived, not guessed.** At level start the board's *true*
minimum solution length is computed by BFS (`board.minSolution()`) and the budget
is `min + slack(level)` (`constants.budgetFor`). This **guarantees every level is
winnable by a perfect player** — a flat formula can't promise that and would ship
unwinnable boards at high levels. The shrinking slack (9 spare moves at L1 → 2 at
L8+) is the difficulty ramp: a run ends when the player can no longer play near
the optimum, never because the puzzle was impossible. If BFS can't decide within
its node cap (essentially never at these sizes) it falls back to a generous bound.
Deals are also DFS-checked for solvability and re-dealt until solvable.

**Content modules (`src/js/content/`):**

- `constants.js` — `colorsFor`/`sparesFor`/`vialsFor`, `CAPACITY`, the
  min-derived budget (`budgetSlack`, `budgetFor`), scoring (`completeScore`,
  `clearBonus`), advance/over delays.
- `board.js` — the vial stacks (bottom→top arrays), `describe(i)`, pour rules
  (`canPour`/`pour`), `isWon`, `snapshot`/`restore` (for undo), `completeCount`,
  `hasMove`, and the solver pair `solvable` (DFS, generation) + `minSolution`
  (BFS, budget). Carries no audio or scoring.
- `game.js` — run FSM (`play` / `levelclear` / `gameover-pending` / `gameover`),
  cursor, the source-select→pour two-step, an undo stack (snapshot + score +
  movesLeft, so undo fully reverts AND refunds the move), move-budget accounting +
  `checkStuck()` game-over, scoring. Emits events; the **screen** turns them into
  audio + announcements.
- `audio.js` — instrument families and the six colour timbres, `playColor`
  (vial-panned, timbre intact), `scanVial` (plays a stack bottom→top), the
  travelling-pan `pour`, pickup/deselect/invalid/complete/undo cues, world-fixed
  `locator` beacon, level/over cues, ambience, and `testDirection`/`demo*`
  diagnostics. `setStaticListener` is a no-op (kept so the shared `#test` screen
  doesn't error).
- `events.js` — shared pubsub.

**Controls (in-game):** move = ← / → (or A/D / numpad / gamepad dpad) · **Enter**
or **Space** (gamepad A) = pick up source / pour · **U** or **Backspace** (gamepad
B) = undo · **C** (gamepad X) = scan current vial · **F1** status
(score/level/moves) · **F2** locate (which vial) · **F3** vials sorted · **Esc**
pause.

**Why pan-only, never pitch-by-position (important):** see decision #2 — a colour
is identified by its timbre+pitch, so shifting pitch for position would make the
same colour sound different in different vials and break the whole game. Position
lives entirely in the stereo pan, which is world-fixed.

**Online scores — REGISTRATION PENDING.** `app/onlineScores.js` uses id `decant`
with a placeholder `SECRET`. Register at `https://scores.oriolgomez.com/admin`
(display_order `desc`, max_score 1000000, meta
`[{"key":"level","type":"int","min":0,"max":999}]`) and paste the secret over
`SECRET`. Until then submissions fall back to local high scores. See
`/home/scores/INTEGRATION.md`.
