...
This repo is **tone48** (formerly "Meld") — an audio-first, blind-accessible
*2048-style slide-and-merge* puzzle built on the `syngen-template` (which sits on the
...
global — see the build-time `/tmp/meld-smoke.js` approach.

This project already has a game (Meld); it is **not** a blank template, so
the "starting a new game" foundational-questions flow below does **not** apply to
ongoing work here.

## This game: tone48

**Pitch.** A 4×4 board of "tones" (each a power of two). Swipe in a compass
direction and every tone slides that way; two equal tones that collide MELD into
one of double the value, a step higher in pitch. After a move that changes the
board, a new low tone appears. Build the highest tone you can; the run ends when
the board is full and nothing can meld. A *thinking* (tactical) game — it is the
2048 puzzle, heard. The board is laid out **spatially around you**: each tile
sounds from its own compass cell and its pitch encodes its value, so a board scan
paints the whole grid around your head.

**Design decisions (the template's required questions, answered):**

1. **Perspective / movement** — static, turn-based. No cursor: you swipe the
   whole board in one of four compass directions. `content/board.js` owns the
   grid + slide/meld rule; `content/game.js` owns the run.
2. **Audio listener — SCREEN-LOCKED (fixed yaw, `LISTENER_YAW = Math.PI/2`),
   parked at the board's CENTRE.** North front, south behind, east right, west
   left. Each tile sounds from its compass cell (`cellOffset`: north-west tile
   front-left, south-east behind-right); a tile's PITCH encodes its value (each
   doubling = +half octave). Screen→audio y-flip in `relativeVector`/`behindness`.
3. **Audio role — audio-first / blind-accessible.** The board is read by ear: a
   spatial **board scan** (each cell from its position, pitch by value) and
   spoken **row scans** (1–4). Two `aria-live` regions; the visual grid is
   `aria-hidden`.
4. **Input — keyboard + gamepad.** Mouse pointer-lock neutered.
5. **Persistence — high scores only** (local `app.highscores` + online). No
   autosave.
6. **Progression — one endless board (score chase).** No levels; the run ends
   when the board is stuck. `state.level` carries the EXPONENT of the best tone
   reached (∈[0,999]) as the online-score meta. FSM screens: menu → game ↔ pause,
   game → gameover → menu; plus help, highscores, language, hidden `#learn` /
   `#test`.
7. **Synth aesthetic — modern / realistic** (warm marimba-ish tone voices, a
   whoosh per move, a rising two-tone meld, a soft pad ambience). No retro waves.

**Content modules (`src/js/content/`):**

- `constants.js` — `SIZE` (4), `FOUR_PROB` (spawn-4 odds), `OVER_DELAY`,
  `MAX_SCORE`.
- `board.js` — the grid + the slide/meld engine (`move(dir)`: `meldLine` compress+
  merge per line, each tone melds once; returns `{changed, melds, gained}`),
  random `spawn`, `canMove` (game-over test), and queries (`maxTile`,
  `maxTileCell`, `emptyCount`, `valueAt`). `_grid` / `_setGrid` are test hooks.
- `game.js` — run FSM (`play` / `gameover-pending` / `gameover`), `move`
  (slide → spawn-on-change → milestone tracking → game-over when `!canMove`),
  `scanBoard` / `scanRow`, scoring. Emits events; the **screen** turns them into
  audio + announcements.
- `audio.js` — the screen-locked binaural scaffolding, `valuePitch`/`cellOffset`,
  the spatial `boardScan` / `rowScan`, `playMove` (whoosh → staggered `meldFx` →
  `spawnFx`), `milestone`, level cues, and `demoLadder` / `testDirection`
  diagnostics.
- `events.js` — the shared tiny pubsub.

**Controls (in-game):** arrows / WASD / numpad = move the **inspection cursor**
one cell (the landed cell sounds from its compass position, pitch by value, and
its value is spoken; edges give a soft bump). **Shift + arrows / WASD / numpad**
= swipe (slide+meld). **C** = scan the board (spatial) · **1**–**4** = scan a row
(spoken) · **F1** score/best · **F2** best tone + where · **F3** free cells +
tones · **F4** last move · **Esc** pause. Gamepad: dpad moves the cursor, **bumper
(LB/RB) + dpad** swipes, A scan, Start pause.

**The spatial board scan is the centrepiece.** Each tone plays from its compass
cell with pitch by value, so the big tones are audibly *placed* around you — keep
the highest in a corner, hear the board fill, and plan the swipe that opens it
back up.

**Online scores — REGISTERED.** `app/onlineScores.js` is registered as game id
`tone48` with its real `SECRET` in place (display_order `desc`, max_score 1000000,
meta `[{"key":"level","type":"int","min":0,"max":999}]` — here `level` is the best
tone's exponent). Any network failure still falls back to the local high-score
table (`tone48-highscores-v1`). See `/home/scores/INTEGRATION.md`.
