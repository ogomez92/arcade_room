;(async () => {
  ...
  // Every cue in content.audio authors its own ADSR + filter tail, so the
  // global convolution reverb (active-by-default and permanently wired into
  // master) only smears them. Kill the send. See the "Reverb on a one-shot"
  // gotcha in CLAUDE.md.
  ...
  // app.autosave stays OFF (default) — Air Hockey persists records out-of-band
  // via app.records, not engine.state, so there's no run-state to autosave.
