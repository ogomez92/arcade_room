app.screenManager = (() => {
  ...
  const machine = engine.tool.fsm.create({
    ...
    transition: {
      none: {
        activate: function () {
          // Honour diagnostic hash routes here, in the none→activate
          // transition (NOT from main.js after dispatch). #test = orientation
          // tester, #learn = cue auditioner; everything else boots to the menu.
          const hash = (typeof window !== 'undefined' && window.location && window.location.hash || '').toLowerCase()
          if (hash === '#test') return this.change('test')
          if (hash === '#learn') return this.change('learn')
          this.change('menu')
        },
      },
    },
