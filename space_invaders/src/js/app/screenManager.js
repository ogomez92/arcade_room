app.screenManager = (() => {
  ...
  const machine = engine.tool.fsm.create({
    ...
    transition: {
      none: {
        activate: function () {
          // Honor #hash for diagnostic routes (e.g. #test, #learn). Per
          // the CLAUDE.md hash-routing gotcha, this is the only safe place
          // to dispatch into a non-default initial screen — once the FSM
          // is past `none`, the hash is too late.
