...
## Horses — game-specific (READ THIS FIRST)

This is the digital adaptation of the classic Spanish *feria* horse-race
arcade. Player at the throwing line, horses race forward, audio-cursor
sweeps left-right across 5 lanes, tap to fire. **The foundational design
questions in the next section have already been answered — do NOT re-ask
them.** The game is built on top of `syngen-template` and reuses its
patterns; the rest of this file is the inherited template guidance.

### Locked-in design

| Decision | Answer |
|---|---|
| Perspective / movement | Player stationary at throwing line; horses race away on a 1D forward track |
| Audio listener | **Screen-locked binaural** (yaw constant `Math.PI / 2`) |
| Audio role | Audio-first / blind-accessible — visuals minimal |
| Input | Keyboard primary; gamepad supported via the template's mappings |
| Throw | Continuous sweeping cursor + tap-to-fire; 5 lanes ascending in value with shrinking width |
| Stamina | Tap-cost (`TAP_COST = 0.07`); recovery `RECOVER_REST = 0.34`/s, halved during the 0.4s after a tap. Depleted player still throws but advance is scaled by `max(stamina, 0.1)` |
| Win | First across `TRACK_LENGTH = 1000` units; ties within 80ms = photo finish |
| Solo | Championship vs AI (5 races, persistent via `engine.state` + `app.autosave`; high scores via `app.highscores` dual backend) |
| Multiplayer | Not yet wired; reserved slug `horses` and the shared `turn.oriolgomez.com` infra |
| Audio | Pure procedural synth, no sampled assets |
| Commentator | aria-live text only, bilingual EN/ES via per-locale phrase pools (NEVER translations) |

### File layout (game-specific)

```
src/js/content/
  audio.js          — synth voices, listener (yaw=PI/2), gallop voices, crowd, organ
  cursor.js         — sweeping cursor (1.6s period), lane-enter cursor tick
  lanes.js          — 5 lanes, values [1,2,3,5,8], pans, shrinking widths
  horse.js          — common horse state shape (player + AI share)
  player.js         — player tap → resolve, stamina drain/recover
  ai.js             — per-AI personality, throw rhythm, lane bias, hysteresis
  race.js           — single-race FSM, photo-finish detection, point ranking
  championship.js   — multi-race season, persistence via engine.state
  commentator.js    — bilingual scheduler, dual aria-live regions
  game.js           — top-level glue, per-frame tick chain

src/js/app/
  i18n.js           — t() + pick(); EN+ES menus + ~30 commentator categories
  highscores.js     — dual backend (Electron JSON / localStorage)
  screen/{splash,mode,championshipMenu,game,raceResult,highscores,learn,test,language}.js
```

### Commentator authoring rules

- **Per-locale phrase pools**, NEVER translations. Spanish is a *feria*
  barker (`¡Carrerón carrerón carrerón!`); English is a county-fair barker.
  Each pool is hand-authored in its register.
- **Storage**: the commentator holds `{categoryKey, params}`, never rendered
  strings. Translation happens at speech time, so a mid-race locale switch
  produces the next line in the new locale's own register.
- **Dual aria-live regions**: `.a-live--polite` for filler / threshold
  crossings; `.a-live--assertive` for state flips (lead change, photo
  finish, win) and status hotkeys (F1-F7).
- **Anti-repeat**: per-locale × per-category, recent-2 indices avoided. If
  the same line still wins the draw twice in a row, the region is cleared on
  `requestAnimationFrame` and re-set so screen readers re-announce.

### Status hotkeys (game screen, all assertive)

- F1 position in field, F2 stamina, F3 distance, F4 gap to leader,
  F5 race time, F6 championship points summary, F7 race index.
- F1 / F3 / F5 use capture-phase `preventDefault` so the browser's Help /
  Find / Reload defaults don't fire.

### Tunables

- Track length: `content.race.TRACK_LENGTH` (1000)
- Races / season: `content.championship.RACE_COUNT` (5)
- Cursor period: `content.cursor.CURSOR_PERIOD` (1.6 s)
- Lane widths / values: `content.lanes.{WIDTHS, VALUES}`
- Stamina: `content.player.{TAP_COST, RECOVER_REST, RECOVER_TAPPING}`
- AI difficulty: `content.championship.difficulty()` ∈ [0, 1] feeds `content.ai.reset(horses, difficulty)`

### Hash routes

- `#test` → listener verification (front/right/behind/left ticks)
- `#learn` → per-voice audition

`localStorage['horses.lang']` is the locale key (per-game so it doesn't leak
across the games hosted on the same origin).

### Multiplayer wire protocol

`content/net.js` is the transport. Star topology, host-authoritative; PeerJS
broker for signalling, direct WebRTC data channels for traffic, the shared
`turn.oriolgomez.com` coturn for STUN/TURN/TURNS. Game slug `horses-`,
4-character codes drawn from `BCDFGHJKLMNPQRSTVWXZ23456789`.

Messages (JSON; `type` is the wire envelope, payload fields use `kind` to
avoid clobbering it):

| direction | type | payload |
|---|---|---|
| c→h | `hello` | `{name, locale}` |
| c→h | `input` | `{t, lane}` (client cursor lane at moment of tap) |
| c→h | `leave` | — |
| h→c | `lobby` | `{peers: [{peerId, name, slot, isHost}], hostId}` |
| h→c | `start` | `{selfId, slots: {peerId: slot}, raceSeed, t0, hostId}` |
| h→c | `snap` | `{t, raceState, progress, crowdLevel, horses: [...], events: [...]}` |
| h→c | `end` | `{order: [{id, slot, finishOrder, finishedAt}], photoFinish, points}` |
| h→c | `kick` | `{reason}` |
| ↔ | `ping` / `pong` | `{t}` (heartbeat — 2s tick, 6s timeout) |

`snap.events` carries `{kind: 'thunk' | 'hit' | 'miss' | 'whinny' |
'photoFinish' | 'end', ...}` — clients re-emit through `content.audio` with
their own listener pose. Local taps push events into `content.race.pendingEvents`
and `getSnapshot()` drains them; the host never fires audio out-of-band.

Lobby flow:
1. Host calls `content.net.host({name, locale})`; PeerJS opens at `horses-<code>`.
2. Client calls `content.net.join({code, name, locale})`; sends `hello`.
3. Host fires `lobby` (locally and over the wire on each peer change).
4. Host clicks Start → `content.net.announceStart()` sends per-peer `start`
   with each client's `selfId` so each knows its assigned `mySlot`.
5. Client's lobby screen catches `start` and transitions to game screen
   with `mode: 'mp-client'`; host transitions itself to `mode: 'mp-host'`.
6. Host runs `race.frame()` at 60Hz, broadcasts `snap` at 30Hz.
7. Client renders snapshot-driven horses + replays `events` through its own
   `content.audio` graph so spatial audio uses the *local* listener.
8. Race finishes → host pushes `end` event into the next snapshot AND sends
   a separate `end` message before clearing state, so clients see the final
   ordering before the game state tears down.

Bot fill: if the lobby is short of `content.race.HORSE_COUNT` (6), the host
fills the remaining slots with AI horses (`_isFiller: true`). Clients
reconstruct the full field from snapshots; bots ride the same `horses[]` as
remote peers.

### Important MP gotchas

- **Trust the client's lane** for v1 simplicity. Both sides run the same
  deterministic cursor (linear-time triangle wave), and the client's
  `input` includes the lane it observed. CLAUDE.md hints at re-resolving
  on the host with host-side cursor; that's a follow-up if it desyncs.
- **Don't put `type` in event payloads.** The wire envelope already uses
  `type`; the payload uses `kind`.
- **Role-guarded mutations**: clients never mutate horse state directly;
  they read it from `snap`. The local player.tap on a client only emits
  the audible thunk and forwards the input to the host.
- **`endIdle()` unsubscribes the net listeners** so a second MP session
  doesn't double-fire.
