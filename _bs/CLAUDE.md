...
7. **Synth aesthetic.** Pleasant tonal synth with pentatonic note layout. Hint = bell-like sine + 5th harmonic; echo = warmer triangle + sub octave + slight saw bite. No samples.

### Note layout (pentatonic, root C major)

Each arrow is one fixed note + pan, defined in `content.audio.NOTE`:

| Arrow | Note | Frequency  | Pan   | Vibe |
|-------|------|------------|-------|------|
| down  | C4   | 261.63 Hz  |  0.00 | low, centred — earth |
| left  | E4   | 329.63 Hz  | -0.70 | warm, panned left |
| right | G4   | 392.00 Hz  | +0.70 | bright, panned right |
| up    | C5   | 523.25 Hz  |  0.00 | high, centred — sky |

These four notes form a C-E-G-C arpeggio when played in order, which is what `audio.success()` and `audio.levelUp()` exploit. Players who can't tell C4 from C5 by ear can still place the cue from the pan alone (left vs. right) plus high/low — every direction has at least two unambiguous distinguishing axes.

### Difficulty curve

Defined in `content/game.js`:

```
bpm        = clamp(70 + 10 * (level - 1), ≤ 140)
measures   = 1 + floor((level - 1) / 3)
beats/measure = 4
notes      = measures * 4   (every beat carries a hint note)
hit window = ±0.4 * beat duration   (40% of a beat on either side)
lives      = 3
```

So levels 1–3 = 1 measure (4 notes), 4–6 = 2 measures (8 notes), 7–9 = 3 measures, etc., with BPM climbing 70 → 80 → 90 → … → 140 (capped). At the BPM cap only the memorize-length keeps rising. The same `floor((level-1)/3)` boundary advances the music arrangement tier (see `content.music`).

### State machine (content/game.js)

```
idle → intro → hint → transition → echo → verdict
                                              └→ intro (next level on clean clear / retry on miss)
                                              └→ gameover (when lives hit 0)
```

Every phase boundary lands on a musical bar so beats never drift relative to the backing track. There are NO scheduled metronome ticks — the procedural drums in `content.music` ARE the click track. There are also no spoken "listen" / "your turn" announcements — the music itself plus the `go()` cue mark the phase changes.

- **intro** (1 measure of music) — accessibility announcement of level / BPM / measures plays over the bed; the intro phase lets the player lock into the new tempo before hints start.
- **hint** (M measures) — drums + bass + pad continue. Hint notes are pre-scheduled on the `audioContext.currentTime` clock at `phaseStart + i * beatDur + 5 ms` so the kick's transient doesn't smear the pitched attack.
- **transition** (1 measure) — drums alone. A single `audio.go()` cue (B4 → C5 leading-tone resolution) is scheduled at `phaseStart + 3.5 * beatDur` so its C5 resolution lands ~10 ms before echo's downbeat. This replaces the old "Your turn" speech announcement entirely.
- **echo** (M measures + half a beat of slack) — drums continue. Player presses must land within `±0.4 * beatDur` of the expected beat to count as a hit.
- **verdict** (1.4 s) — fanfare on clean clear, fail blip on miss; advance to next level / retry / game-over.

Same pattern repeats on a retry (so the player can learn it). New random pattern on advance. The pattern generator avoids three of the same arrow in a row (`randomPattern` in `content/game.js`) — easy patterns get tedious, hard ones stop being audibly distinguishable.

### Audio architecture

Two content modules:

**`content.audio`** — short one-shot cues that layer over the music bed:
- `hint(direction, when?)` and `echo(direction, when?)` — the two arrow timbres. Same frequency + pan but different waveform / harmonic structure so the player can tell their own playing apart from the cue they're echoing.
- `go(when?)` — the listen→echo "go" cue. A B4 → C5 leading-tone resolution. The note is OUTSIDE the four-arrow pentatonic so it sounds distinct from any hint, and its semitone tension naturally pulls toward C5 (the up-arrow's note). Centre-panned and slightly louder than a hint so it cuts through the music.
- `tick(beatIndex, when?)` — kept for backwards compat; not currently used by the game (drums replace it).
- `success`, `fail`, `levelUp`, `gameOver` — short arpeggios composed from the same four arrow voices.
- `now()` — passthrough to `audioContext.currentTime`.

**`content.music`** — continuous procedural backing track. Audio-clock lookahead scheduler running off `engine.loop.on('frame', tick)`; refills a 150 ms queue of upcoming 16th-step events, then stops gating on `state.running`. Voices: kick (sine 110→38 Hz sweep + click), snare (200 Hz triangle + bandpassed noise), hat (highpassed noise burst), bass (triangle + detuned sine, low-passed at 420 Hz), pad (3-voice chord octaves above the bass, slow filter sweep). Master bus at 0.35 with a 200 ms fade-in / 400 ms fade-out so transitions don't pop.

Voicing keeps the bed out of the C4–C5 arrow band: bass C2–C3, pad C3–G3, drums broadband transients. Hints and echoes own the C4–C5 register.

Per-level arrangement is picked by `floor((level-1)/3)` — the same boundary as the memorize-length curve, so the music thickens as the memorization gets harder:
- tier 0 (lvl 1–3): kick on 1, snare on 3, no hat, no pad, single chord (C)
- tier 1 (lvl 4–6): standard kick-snare, hi-hats on the e of every beat, soft pad, I–V–vi–IV
- tier 2 (lvl 7–9): syncopated kick, ghost-snare on 16, 8th hats, I–IV–V–V
- tier 3 (lvl 10+): minor mood, 16th hats, vi–IV–I–V

`content.music` exposes `start({bpm, level})`, `stop()`, `setBpm(bpm)`, `setLevel(level)`, `nextDownbeat()` (audio-clock time of the next bar boundary, useful if you want to align further events to the grid), and `bpm()` / `level()` getters. The game module calls these from `start()`, `enterIntro()` (per-level updates), and `enterGameOver()` / `stop()`.
...
- **Variant audio palettes** — a soundtest screen ("press T from menu") that lets players audition alt timbres for the four arrows (chiptune / soft pad / FM bell). Then store the choice in `app.settings`.
- **Note density** — add a difficulty knob: at low levels only beats 1 and 3 carry notes (call beats 2 and 4 "rests" and skip the hint scheduler for them). Raises the floor and stretches the curve.
...
When in doubt about wiring screens, audio scheduling, or timing-window semantics, the implementation is small (`content/audio.js`, `content/game.js`, `app/screen/game.js`) — just read it.
