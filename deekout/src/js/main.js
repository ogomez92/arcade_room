;(async () => {
  ...
  // Every cue authors its own ADSR + lowpass tail, so kill syngen's global
  // reverb send (otherwise one-shots smear). See CLAUDE.md gotcha.
  ...
  if (!app.isElectron()) {
    window.addEventListener('beforeunload', (e) => {
      if (!engine.loop.isPaused()) {
        // preventDefault() alone triggers the browser's leave-confirmation
        // dialog; the legacy (deprecated) returnValue string is ignored anyway.
        e.preventDefault()
      }
    })
  }
