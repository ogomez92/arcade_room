...
This repo is **Super Deekout** — an audio-first, blind-accessible arcade coin-chase remake built on the `syngen-template` (which itself sits on the [syngen](https://github.com/nicross/syngen) Web Audio engine). Ships as an HTML5 app served from `public/` (also Electron-capable). The template scaffolding docs are retained below the game section.

There is no test suite, no linter, and no `npm` scripts. All tasks run through Gulp (use **pnpm/pnpx**). Build with `pnpx gulp build` (`--debug` for unminified). Do not start a dev server in this checkout — verification stops at build/static checks; `node --check public/scripts.min.js` confirms the bundle parses.

## This game: Super Deekout

A game-specific `CLAUDE.md` now exists, so the "Starting a new game" foundational questions below DO NOT need re-asking. The locked design:

- **Field:** 35×35 grid (`content/constants.js` `GRID`). Integer cells `0..34`; player/movers hold fractional `(col,row)`. `row` increases SOUTH, `col` increases EAST.
- **Movement:** NO momentum. Hold arrow(s) → roll at constant speed; release → stop and go silent. Diagonals from two held arrows. Speedups raise speed; `Ctrl` forces base speed. The game screen (`app/screen/game.js`) reads RAW `keydown`/`keyup` for movement + every action key; `app.controls.game()` is unused for gameplay.
- **Audio:** SCREEN-LOCKED. Listener is pinned at the origin facing north for the whole game (`content/audio.js` `setStaticListener`); the player is always centred. A source at world `(col,row)` relative to player `(pc,pr)` maps to the binaural frame (`+x`=forward, `+y`=LEFT) as `{ x: pr-row, y: pc-col }` (`offsetAudio`/`relAudio`). North→front, east→right, south→behind (muffled + detuned down via `behindness`+`applyBehind`), west→left. **Verify with the `#test` route after any listener/coord change.** Per-instance pitch jitter (`audio.jitter`) keeps same-type items distinguishable. Because HRTF nulls are a weak L/R cue on speakers, every spatial voice ALSO rides a `StereoPannerNode` (bearing-sine pan via `panFor`: due-west→hard left, due-east→hard right, north/south→centred) summed with the binaural ear — this is the dominant left/right cue, so a beacon does not need to be bright (high-pitched) to be locatable. **Note:** coordinates passed to the audio layer are raw tile units (no `tileToM`/meter scaling); the generic syngen `tileToM`/yaw snippet in the Gotchas section is template background, not this game's `setStaticListener` (fixed yaw 0).
- **Synth:** modern. Each cue authors its own ADSR + lowpass tail; the global syngen reverb send is disabled in `main.js`.
- **Music:** `content/music.js` — procedural lookahead scheduler; `setLevel(n)` raises BPM + layers and shifts major→minor as levels climb.
- **Difficulties:** easy / normal / crazy, all unlocked (`DIFFICULTY_TABLE` + `levelParams()`).
- **Scores:** local per-difficulty top-10 (`app/highscores.js`, key `deekout-highscores-v1`) + online submit to `scores.oriolgomez.com` (`app/onlineScores.js`, GAME_ID `deekout`). Online fails soft until the game id is registered with the admin endpoint. Nickname in `localStorage 'deekout.nickname'`; difficulty in `localStorage 'deekout.difficulty'`.

### Audio voice plumbing (`content/audio.js`)
Two factories build every game sound; reach for these instead of wiring nodes ad hoc:
- `makeProp({build, col, row, gain, maxDistance, power, gainModel})` — a **looping** spatial voice. Chain is `out(gain) → muffle(lowpass) → {binaural ear + StereoPanner}`. Returns `{setPosition, setGain, applyBehind, destroy}`. The optional `gainModel` overrides the default exponential binaural distance model (e.g. `engine.ear.gainModel.normalize` for an off-screen indicator that should stay audible anywhere) — but note the **StereoPanner path has no distance model of its own**, so the per-frame `setGain` (on `out`) is what actually fades a voice with distance; coins, the bot, and items all rely on a falling `setGain` rather than the binaural model alone. The `build(out, ctx, detuneSignal)` callback returns **either** an array of stop-fns **or** `{stops, controls}` — `controls` is merged onto the returned prop, which is how a voice exposes extra per-frame setters (e.g. the robot's `setRoll(0..1)` drives its per-step "chug" pulse cadence/intensity + the wheel-roll hiss from its actual movement). Looping voices that need a rhythm (the coin metallic-bounce beacon, the robot's chug) run a tiny audio-clock lookahead scheduler inside `build` and tear it down via a stop-fn. `applyBehind(amount)` closes the muffle + detunes down for south/behind sources; callers feed it `behindness(col,row)` each frame.
- `spatialOneShot(world, build, {duration, ...})` — a disposable positioned event (same dual path + a static behind-muffle + distance attenuation), auto-torn-down after `duration`. `nonSpatial(build)` is the same but straight to the master mix for global stings.
- Helpers: `envelope` (ADSR), `tone` (one detuned osc), `bellPartial`/`metalNote` (struck-metal partials with exponential decay — coins/chimes), `jitter` (deterministic per-id pitch spread). `silenceAll()` sets a 60 ms suppression flag (one-shots only); looping voices are stopped per-module via each module's own `silenceAll()`.

Looping-voice modules (`coins`, `enemies`, `items`, `experiment`) each own a `Map` of voices and a `silenceAll()`. (The **player makes no movement sound** — its old wheel-roll voice was removed; `player.silenceAll()` is now a no-op kept for the FSM.) **On game over `game.gameOver` silences every one of them individually** — do NOT call the global `game.silenceAll()` mid-run, because that also `music.stop()`s and nothing restarts the soundtrack. A non-fatal hit does **not** silence anything (play continues). Learn-screen previews of looping voices spin up a throwaway voice under a reserved id (`enemies.preview`, `items.previewNasty/previewGood`) that the per-frame loop never touches, then `setTimeout`-destroy it.

### Module map (`src/js/content/`)
`constants` (data/enums/tables) · `state` (3-tier state: `career` cross-level, `level` ephemeral, `player`) · `field` (grid/walls/fusion/spawn cells) · `audio` (screen-locked listener + all SFX) · `music` · `player` (no-momentum move; **silent** — no movement voice) · `coins` (looping metallic-bounce beacons; **smart** nearest-only voicing — cap 3 in ALL mode / 1–5 in SINGLE, rank-attenuated so the closest coin dominates and the rest are faint hints; auto-pickup within `PLAYER.pickupRadius`; rapid-collect within `rapidCoin.{n,windowS}`→good item — the ONLY good-item trigger) · `enemies` (1O1 robot chaser + rocket; positions/speed persist; speed scales with PEAK level so level-drop never slows them) · `bullets` (rocket projectiles) · `items` (good + nasty; bombs, hazards, steal-time, level-drop) · `experiment` (numbered pieces asc→inventory / desc→oil slicks; inventory E/C/W/S; oil-slick collisions) · `bonus` (Coin Shower, Mine Field) · `scoring` (awards + Health/Time/Item end-of-level bonuses) · `announcer` (polite/assertive + status hotkeys + optional TTS) · `game` (top-level FSM + per-frame order).

### Game FSM (`content/game.js`, `state.data.fsm`)
`intro → play → …`, plus `hit`, `bonus`, `paused`, `gameOver`. **Play never freezes between levels.** Coins gone or allowed early-end (Space, ≤2 coins or coin-spawn active) → `clearLevel` (awards Time/Health/Item bonuses, all non-blocking audio/aria-live) → `afterClear` → if `isBonusLevel` (every level ≡5 mod 10) the `bonus` round, else `advanceLevel` straight into the next level's `play`. The `ready` and `levelClear` freeze phases still exist in `STATE` but are **bypassed** — `beginLevel` is always called with `{skipReady:true}` and `clearLevel` calls `afterClear` immediately — so the only stop is a manual `paused`.

**No lives — health is the only survival resource.** Every hit subtracts `health`: contact hits (robot/rocket catch, bullet, oil slip) route through `hitPlayer` (damage from `constants.HIT_DAMAGE`), bombs/hazards/walls apply their own damage via `player.applyDamage(amount, cause)`. A non-fatal hit plays the cause's sting, knocks enemies back, opens a ~1.3s mercy window, and runs a brief `hit` recovery beat (~0.6s) before resuming `play`. When `health ≤ 0`, `gameOver(cause)` silences all voices, sets `pendingGameOver`, keeps the audio tail ticking ~2.4s, then dispatches the `gameover` screen. The death cause is attributed via `lastHitCause` (set by `hitPlayer` and `game.noteHitCause`, which `player.applyDamage` calls).

Cross-level persistence: `resetLevel()` rebuilds coins/timers/per-level counters but **carries over the player's position, in-flight items (good/nasty/bombs/hazards/oil + their ids), and never touches enemies, score, or health** — nothing snaps back to the centre on a level change. Item voices keep playing across the transition (`clearLevel` does NOT silence items; `items.reset()` runs only on a fresh career).

**Spawn / hit safety:** enemies persist their positions across levels, and the player no longer recentres on a level change, so a bot could be right next to you when a level starts. `beginLevel` grants a mercy `player.invisibleUntil` (`READY_S + ~1s` ≈ 2.6s of `play`, since there's no `READY` freeze) covering that window. On a non-fatal hit, `knockbackEnemies` shoves every bot `KNOCKBACK` (9) cells directly away from the player — with a corner-direction fallback if a bot is sitting exactly on the player (zero away-vector) — and the ~1.3s hit mercy prevents chain-draining.

**Gameplay tuning** (coin counts, enemy speed, nasty-item cadence `firstAtS`/intervals, `rapidCoin`, wall damage) lives entirely in `constants.js` `DIFFICULTY_TABLE` per difficulty, resolved by `levelParams(difficulty, level)`. Per-contact-hit health loss is `constants.HIT_DAMAGE` (robot/rocket 30, bullet/oil 25); bombs/hazards/walls apply their own. Pickup/grab radii are `PLAYER.pickupRadius` (coins) and per-module constants (`items` `GRAB_RADIUS`, `experiment` `PIECE_RADIUS`); free-cell spawns inset 1 cell off the boundary (`field.randomFreeCell` default `margin`) so nothing lands on a wall. This is an audio-first game — these radii are deliberately generous so the player can centre by ear.

### Keymap (handled in `app/screen/game.js`)
Arrows move (diagonals); Ctrl=normal speed; E/C/W/S inventory; Space early-end; I inventory; Enter coins+health; Shift score+level (double-tap=digit-by-digit); H high score; T time; M coin mode; 1-5 nearest-coin count; P pause; Esc→menu; F1-F4 redundant status reads.

### Verification harness
Without a browser, the integration path is checked headless: stub `engine`/`app`/Web-Audio, eval the `content/` modules in concat order, then drive `content.game` through clear/bonus/experiment/death. (Reference: the harness used during the build — re-create it under `/tmp` to smoke-test logic changes.)
...
This is a **secure system**: plain `npm`/`npx` are disabled. Use **pnpm** /
**pnpx** for everything (`command npm ...` would bypass the block — don't;
prefer pnpm so the lockfile and store stay consistent).

```sh
pnpm install               # install deps (must run first; Gulpfile expects node_modules/syngen)
pnpx gulp build            # one-shot build of public/scripts.min.js + public/styles.min.css
pnpx gulp watch            # rebuild on src/** changes
pnpx gulp serve            # do not use for testing here; folder is already served independently
pnpx gulp dev              # do not use for testing here; folder is already served independently
pnpx gulp electron         # launch Electron against current public/ build
pnpx gulp electron-rebuild # build then launch Electron
pnpx gulp dist             # build + package electron-packager + zip HTML5 build into dist/
```
